...
@section('content')
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="r">共有数据：<strong>{{$refusedPosts->count()}}</strong> 条</span>
    </div>
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <tbody>
        @foreach($refusedPosts as $post)
        <tr class="text-c">
            <td><input type="checkbox" value="{{$post->id}}" name="refusedPosts[]"></td>
            <td>{{$post->id}}</td>
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                if(data.error==0){
                    ...
                    layer.msg(text,{icon:6,time:1000});
                }
            }
        });
    }
